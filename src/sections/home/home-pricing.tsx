...
export function HomePricing({ sx, ...other }: BoxProps) {
  ...
  const renderDescription = () => (
    <SectionTitle
      ...
      description="Professional home and property services with transparent pricing. Book online and get instant confirmation."
...
function PlanCard({ plan, sx, ...other }: PlanCardProps) {
  const standardLicense = plan.license === 'Cleaning Services';

  const plusLicense = plan.license === 'HVAC Services';
...
const PLANS = Array.from({ length: 3 }, (_, index) => ({
  license: ['Cleaning Services', 'HVAC Services', 'Renovation Services'][index],
  price: [44, 69, 199][index],
  commons: [
  ...
  options: ['Home cleaning', 'Move-out cleaning', 'Deep cleaning', 'Commercial cleaning'].slice(
    0,
